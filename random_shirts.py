...
def prompted_run(prompt, title, test=False):
    ...
    if test:
        ...
        sample_prompt = prompt.replace(
            "mascot", "University of " + "Washington Huskies"
        ).replace("school", "Washington")
    ...
    for team_mascot in all_tm:
        ...
        if (
            team_mascot[0:2]
            in ["UC", "UN", "UA" "LS", "BY", "NC", "SM", "TC", "UM", "US"]
        ) or (team_mascot[0:3] in ["UT ", "UTE"]):
            full_prompt = prompt.replace("mascot", team_mascot).replace("school", team)
        else:
            full_prompt = prompt.replace(
                "mascot", "University of " + team_mascot
            ).replace("school", team)
        ...
        if main_image is not None:
            ...
            shop_config["title"] = (
                title.replace("school", team).replace("School", team).title()
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p",
        "--prompt",
        help="prompt for design, mascot will be replaced with the mascot from every school",
        required=True,
        dest="p",
    )
    ...
    print(
        f"Sleeping for 5 mins before organizing store to give time for images to upload"
    )
    time.sleep(60 * 5)
    organize_store("rand")
