...
def build_design(config,test=False):
    ...
    score_font = ImageFont.truetype(r'./fonts/Freshman.ttf', size = 70,layout_engine = ImageFont.Layout.BASIC)

    date_font = ImageFont.truetype(r'./fonts/alarm_clock.ttf', size = 25, layout_engine= ImageFont.Layout.BASIC) # autosize this
    ...
    img_draw.text((l1,t1+lh+5),team1,team1color,font=mainfont, stroke_width=1,stroke_fill="black")

    img_draw.text((lv,tv+lh+5),'VS',(255,255,255),font=mainfont, stroke_width=1,stroke_fill="black")

    img_draw.text((l2,t2+lh+5),team2,team2color,font=mainfont, stroke_width=1,stroke_fill="black")

    img_draw.text(((W-wd)/2,ih+(logo2.size[1]//2)-(hd//2)),gamedate,(255,255,255),font=date_font, stroke_width=1,stroke_fill="black")

    if config['game']['done']:
        img_draw.text(((W//4),bv+h1),str(score[0]),team1color,font=score_font, stroke_width=1,stroke_fill="black")
